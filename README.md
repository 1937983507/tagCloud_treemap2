# TreeMap - 地名标签云（tagCloud_treemap） V2.0

本项目作为一个面向空间路径分析的可视化系统，将地名标签、TreeMap 剖分和路线语义紧密耦合，为研究者和数据分析师提供“绘制线路 → 精准筛选 POI → 生成标签云”的一站式工作台。项目对交互流程、视觉调优和导出能力做了深度设计，可作为空间旅游推荐、交通线路讲解、地理教学等场景的通用可视化组件。
...
🔗 [访问在线网站](https://www.hubutagcloud.cn/treemap/)
...
- **数据管线自动化**：基于 `@turf/turf` 和 GeoJSON 判定路径穿越城市，从 `chinapoi.csv` 中筛选城市景点并按照排名映射字号权重。
- **多算法标签云**：内置 7 种 TreeMap 布局（Resquarify、Binary、Dice、Slice、Strip、Spiral、Pivot）与 d3-cloud 词云，结合自研邻接着色与路径线型渲染。
- **可视化可编排**：字体、配色、布局、路径线型均以卡片式面板配置，实时写入 Pinia Store 并驱动画布更新。
- **导出与分享**：SVG 渲染、PNG/JPEG 导出、画布尺寸可调、Loading 遮罩与多状态提示，确保结果在汇报、论文和 Demo 中可复用。
...
- **框架**：Vue 3 `<script setup>` + Pinia + Vue Router（后续可扩展）
- **UI / 交互**：Element Plus、Intro.js、Sass
- **数据 & 可视化**：d3、d3-cloud、自研 TreeMap 算法扩展（Strip/Spiral/Pivot）、@turf/turf
...
tagCloud_treemap2/
├── public/
│   ├── data/               # POI & 行政区数据（CSV / GeoJSON）
│   └── img/                # TreeMap 缩略图、起止点图标、LOGO 等
├── src/
│   ├── assets/styles/      # 全局样式 (SCSS)
│   ├── components/
│   │   ├── content/        # PoiMap、LocationGraph 等数据面板
│   │   ├── tagcloud/       # TagCloudCanvas 及导出逻辑
│   │   ├── layout|color|typeface|line  # 交互控制面板
│   │   └── layout/HeaderBar、FooterBar、SideMenu…
│   ├── stores/poiStore.js  # Pinia 数据中心
│   ├── utils/              # TreeMap 算法扩展、GeoJSON Loader、城市名归一化
...
- 若高德地图图层加载缓慢，可在 `PoiMap.vue` 中关闭 HeatMap/MassMarks 或降低 `densifyPath` 精度。
...
实验室官网详细介绍请参阅：
🔗 [湖北大学制图组官网](https://hubutagcloud.cn/cxq-group/)
